...
Thus, if we had obtained a large p-value in our study, the appropriate conclusion would be
...
A regression line is fit to the shuffled data, shown below. The blue line indicates the slope of the regression line. The red line indicates the regression line for the actual lakes in the sampe, which has a slope of -0.15.  

```{r}
ggplot(data=Shuffle1df, aes(x=Shuffled_pH, y=Mercury)) + 
  geom_point() + stat_smooth(method="lm", se=FALSE) + 
  xlim(c(3, 10)) + ylim(c(0,1.5)) + 
  geom_abline(slope=-0.1523, intercept=1.5309, color="red")
```
...
Notice that we're already seeing F-statistics more extreme (bigger) than the 0.60 F-statistic we saw in our actual data. 
...
**p-value:** Proportion of simulations resulting in value of F more extreme than -0.15 
