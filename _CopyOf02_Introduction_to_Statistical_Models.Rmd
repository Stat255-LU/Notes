...
```{r}
First3Houses <- Houses %>% select(price, waterfront, sqft_living) %>% head(3)
kable(First3Houses)
```
...
Residplot_MR <- ggplot(data=Houses, aes(x=sqft_living, y=price, color=waterfront)) + geom_point()+
              ...
              geom_segment(aes(xend = sqft_living, yend = M_Nbhd_SqFt$fitted.values), color="red")+theme_bw()  + theme(legend.position = "none")
```
