...
We could also use other functions of explanatory variables (for example, log, square root, or a trigonometric function) if we have reason to do so. Like polynomial terms, these can improve fit, but also make the model more complicated and harder to interpret, and should only be used if there is strong reason to do so.  
